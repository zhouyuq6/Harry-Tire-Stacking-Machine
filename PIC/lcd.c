...
void newLine(int lineNum){
    if(lineNum==0) {//first line to second
        lcd_set_ddram_addr(LCD_LINE2_ADDR);
        return;
    }
    else if(lineNum==1) {//second to third
        lcd_set_ddram_addr(LCD_LINE3_ADDR);
        return;
    }
    else if(lineNum==2) {//third to fourth
        lcd_set_ddram_addr(LCD_LINE4_ADDR);
        return;
    }
    else if(lineNum==3) {//fourth to first
